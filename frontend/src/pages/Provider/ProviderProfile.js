  ...
  useEffect(() => {
    const fetchProviderProfile = async () => {
      try {
        ...
        if (fetchedProvider.location) {
          const providerLocation = {
            lat: parseFloat(fetchedProvider.location.latitude),
            lon: parseFloat(fetchedProvider.location.longitude),
          };
          localStorage.setItem(
            "providerLocation",
            JSON.stringify(providerLocation)
          );
          toast.success("Provider location stored successfully!");
        }
      } catch (err) {
      ...
      <div className="max-w-5xl mx-auto bg-white shadow-xl rounded-lg overflow-hidden">
        <div className="px-6 py-8">
            ...
            {provider.location && (
              <div className="bg-gray-50 p-4 rounded-lg shadow-lg">
                ...
                <div className="mt-4 space-y-2 text-lg">
                  ...
                  <p>
                    <strong>Latitude: </strong> {provider.location.latitude}
                  </p>
                  <p>
                    <strong>Longitude: </strong> {provider.location.longitude}
                  </p>
                </div>
              </div>
            )}
          ...
          {provider.totalRating && (
            <span className="mt-4 flex text-lg font-semibold text-gray-700">
              Total Rating -{" "}
